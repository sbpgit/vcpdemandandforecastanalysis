sap.ui.define([
    ...
    return Controller.extend("sacforecastorder.sacforecastorder.controller.View1", {
        ...
        onMRPFilters: function () {
            ...
            if (sSelectUniqueId.length > 0 && !that.isSelstAll) {
                baseFilter += ` and (${unique})`;
            }
            if (sSelectMRPG.length > 0) {
                baseFilter += ` and (${mrpg})`;
        ...
        onApplyFilters: async function (oEvent) {
            ...
            var sProduct = oView.byId("idProduct").getSelectedKey() !== "" ? oView.byId("idProduct").getSelectedKey() : oView.byId("idProduct").getSelectedItem().getText();
            var oStartDate = oView.byId("DRS1").getDateValue();
            var oEndDate = oView.byId("DRS1").getSecondDateValue();
            if (oView.byId("idProduct").getSelectedKey() == "") {
                oView.byId("idProduct").setSelectedKey(sProduct);
            }
            // Validate
            ...
            oModel.read("/getCirGen", {
                ...
                error: function (oError) {
                    ...
                    sap.m.MessageToast.show("Error fetching Data", oError);
                    that._bInitialLoadInProgress = false;
                    that._loadCirGenChartAndCharacteristics(sLocation, sProduct, oStartDate, oEndDate, []);
                }
            });
        ...
        loadCharValChart: function (sLocation, sProduct, oStartDate, oEndDate, aUniqueIds) {
            ...
            oModel.read("/getPlannedOrdAnalysis", {
                ...
                success: function (oData) {
                    ...
                    if (!that.changeCharVAl) {
                        ...
                        if (oCharValCtrl) {
                            ...
                            // aFormattedCharValues.sort()
                            aFormattedCharValues.sort((a, b) => a.text.localeCompare(b.text));
                ...
                error: function (oError) {
                    ...
                    this.showErrorMessage("Error fetching data:", oError);
                    this.showBusyIndicator(false);
                }.bind(this)
            });
        },
        ...
        loadCharacteristics: function (aSelectedConfigProducts, aUniqueIds) {
            ...
            oModel.read("/getPlannedOrdAnalysis", {
                urlParameters: {
                    $apply: `filter(LOCATION_ID eq '${sLocationId}' and CONFIGURATION_PRODUCT eq '${sSelectedConfigProduct}' and PRODUCT_ID eq '${sProducts}')/groupby((CHAR_DESC,CHAR_NUM))`,
                ...
                success: function (oData) {
                    ...
                    aFormatted.sort((a, b) => a.text.localeCompare(b.text));
                    // Prepend "Select All"
                ...
                error: function (oError) {
                    ...
                    this.showErrorMessage("Error loading characteristics:", oError);
                    this.showBusyIndicator(false);
                }.bind(this)
            });
        },
        ...
        _loadCharacteristicValues: function (aSelectedCharacteristics) {
            ...
            oModel.read("/getPlannedOrdAnalysis", {
                ...
                success: function (oData) {
                    ...
                    // Prepare data
                    aValues.sort((a, b) => a.CHAR_CHARVALUE.localeCompare(b.CHAR_CHARVALUE));
                    var aListData = [];
                    ...
                    aListData.unshift({ key: "Select All", text: "Select All" });
        ...
        loadForecastChart: function () {
            ...
            oModel.read("/getAsmbReqAnalysis", {
                ...
                error: function (oError) {
                    ...
                    that.showErrorMessage("Error loading data:", oError);
                    that.showBusyIndicator(false);
                }
            });
        },
        ...
        onAppFilter: function (oEvent) {
            if (this.byId("idLocation").getSelectedKey().length == 0 || this.byId("idConfigProduct").getSelectedKey().length == 0 || this.byId("idProduct").getSelectedKey().length == 0 || this.byId("idCharacteristics").getSelectedItems().length == 0) {
                that.showBusyIndicator(false);
                return;
            }
            try {
        ...
        MRP: async function (oEvent) {
            ...
            ///////////////////////////////////
            let sApply2 = `filter(${baseFilter})/groupby((ASSEMBLY_DESCRIPTION))`;
            const asmbData = await that.readModel("getAsmbReqAnalysis", {
                $apply: sApply2,
                $top: 100000,
            });
...
/////////////////Error Descriptions change 11-11-2025///////////////
/////////////////Assembly  change 12-11-2025///////////////
//////////////////// Test Deploy //////////////////////////